...
class taxonomy_list_widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		//Widget
		$widget = $taxonomy_list_widget_plugin->render_list( $instance, $this->number );
			...
			<?php
				...
				?>
					<li>
						...
						<input type="text" style="width:100%;" name="<?php echo $this->get_field_name( 'delimiter_custom' ); ?>[<?php echo $key; ?>]" id="<?php echo $this->get_field_id( 'delimiter_custom' ); ?>_<?php echo $key; ?>" class="small-text code" value="<?php if( isset( $custom_delims ) && array_key_exists( $key, $custom_delims ) ) echo htmlspecialchars(stripslashes( $custom_delims[ $key ] )); ?>" />
					</li>
				<?php
				endforeach;
			?>
